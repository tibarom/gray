...
export function Members({ Members: initialMembers }: { Members: any }) {
  ...
  return (
    <Suspense fallback={null}>
      <main className="max-w-2xl font-mono m-auto mb-10 text-sm">
        ...
        <List Members={Members} sort={sort} />
      </main>
    </Suspense>
  );
}

function List({ Members, sort }: { Members: any, sort: any }) {
  ...
  return (
    <ul>
      {sortedMembers.map((post, i: number) => {
        const year = getYear(post.date);
        const firstOfYear =
          !sortedMembers[i - 1] || getYear(sortedMembers[i - 1].date) !== year;
        const lastOfYear =
          !sortedMembers[i + 1] || getYear(sortedMembers[i + 1].date) !== year;
